...
from .models import Profile, Post, LikePost, FollowersCount, messg
...
def signup(request):
    ...
    if request.method == 'POST': 
        ...
        if username =="":
            messages.info(request, 'Enter a Username')
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                #log user in and direct to seting page
...
def message(request):
    ...
    user_profile = Profile.objects.get(user=user_object)
    messages =  messg.objects.all()

    if request.method == 'POST':
        msg = request.POST['txtmsg']
        sender = request.user.username
        reg = messg(msg=msg, sender=sender)
        reg.save()
        # txtmeg = NULL
